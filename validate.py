import logging, os, argparse, pathlib, json, sys
...
def validate_schema(input_file, is_parameters):
    ...
    if is_parameters:
        schema_file = open(PARAMETERS_SCHEMA)
    else: 
        ...
        schema_file = open(DATATYPES_SCHEMA)

    # load in the schema and the file to validate
    json_schema = json.load(schema_file)

    file_to_validate = json.load(open(input_file))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Validate a schema that was created with an input CSV file.')
    parser.add_argument('inputfile', help='The schema file to process')
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('-p', '--parameters',  help='The file to be validated is a parameters.json file', action='store_true')
    group.add_argument('-d', '--datatypes', help='The file to be validated is a datatype file', action='store_true')
    ...
    logging.info(f"File {args.inputfile} was successfully validated.")
